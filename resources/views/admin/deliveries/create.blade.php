...
@section('content')
<h1 class="text-xl font-semibold mb-4">Add delivery task</h1>

<form method="post" action="{{ route('admin.deliveries.store') }}" class="space-y-3 max-w-xl">
    @csrf
    ...
    <div>
        ...
        <select name="volunteer_id" class="border rounded w-full px-2 py-1">
            @foreach($volunteers as $v)
                <option value="{{ $v->id }}" @selected(old('volunteer_id') == $v->id)>{{ $v->name }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <select name="donation_id" class="border rounded w-full px-2 py-1">
            @foreach($donations as $d)
                <option value="{{ $d->id }}" @selected(old('donation_id') == $d->id)>#{{ $d->id }} - {{ $d->food_type }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <input name="pickup_location" value="{{ old('pickup_location') }}" class="border rounded w-full px-2 py-1" />
    ...
    <div>
        ...
        <input name="dropoff_location" value="{{ old('dropoff_location') }}" class="border rounded w-full px-2 py-1" />
